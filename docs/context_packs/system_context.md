...
* `configs/` — repo mirror of static S3 configs used by the job (only if the user enforces this convention).
...
The user manually updates the script version in AWS Glue.
...
* No “assumption presented as fact”. Anything not provable from provided code/docs must be labeled TBD or as an explicit assumption (assumptions require the user’s approval before they are used as a basis for further decisions).
...
ChatGPT in chat does not automatically have access to the user’s private GitHub repo contents.
To avoid drift:

* In chat, the user provides files by attaching them or pasting relevant excerpts when asking for analysis.
* In Codex tasks (with repo access), Codex is instructed to read specific repo paths (TARGET_SCRIPT + referenced standards).

---

## When and how this document may be updated

This file may be updated only when the user explicitly decides to change:
