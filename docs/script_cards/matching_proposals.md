...
## Purpose
- Generates category matching proposal outputs in JSON based on the prepared vendor input dataset and writes results to the configured S3 output locations.
...
- Input 1:
  ...
  - format: json
  - required: TBD
  - meaning: Prepared vendor input dataset for category mapping.
...
- Output 1:
  ...
  - format: json
  - required: TBD
  - meaning: Category matching proposal output generated by the job.
  - consumers: TBD
- Output 2:
  - bucket: ${OUTPUT_BUCKET}
  ...
  - format: json
  - required: TBD
  - meaning: Subset of the category matching proposal output derived by the job.
  - consumers: TBD
...
- Validate presence of expected mapping data and handle empty/missing cases.
- Aggregate vendor category data with PIM category match information.
- Write the main proposal output to S3.
- Write a filtered oneVendor_to_onePim subset output to S3, excluding assignment_confidence values of "mixed" or "null".

## Invariants (As Implemented Today)
- TBD

## Failure Modes and Observability
- Failure conditions (as coded): Glue argument parsing failures; S3 head/list/put errors; Spark JSON read failures beyond fallback; unhandled exceptions during processing.
